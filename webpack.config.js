...
// export
module.exports = {
  // 경로에서 파일의 확장자를 기입하지 않아도 읽을 수 있도록 설정
  resolve: {
    extensions: ['.js', '.vue'], // js, vue 파일을 설정.

    // 경로 별칭 사용
    alias: {
      '~': path.resolve(__dirname,'src'), // ~를 '/src/'로 지정 
      'assets': path.resolve(__dirname,'src/assets') // assets를 '/src/assets/'로 지정 
    }
  },
  ...
  module: {
    rules: [
      ...
      // 이미지 파일 사용을 위한설정.
      {
        test: /\.(png|jpe?g|gif|webp)$/, // 왼쪽에 명시한 파일 중 하나라도 매칭이 되면,
        use:'file-loader' // file-loader를 사용
      }
